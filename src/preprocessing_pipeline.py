#!/usr/bin/env python3
"""
EEG Preprocessing Pipeline using MNE-BIDS (Preprocessing Only).
...
through a YAML configuration file. This module focuses on preprocessing steps only
and can save intermediate results for later report generation.

Main Components
---------------
- PreprocessingPipeline: Core pipeline class for preprocessing
...
Output:
  ...
  - save_intermediate_results: Save preprocessing results for report generation
...
class PreprocessingPipeline:
    def __init__(self, bids_root: Union[str, Path], output_root: Union[str, Path] = None, config: Dict[str, Any] = None):
        ...
        self.step_functions = {
            ...
            'save_intermediate_results': self._step_save_intermediate_results,
        }
    ...
    def _step_save_intermediate_results(self, data: Dict[str, Any], step_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        Save intermediate preprocessing results to disk for later report generation.
        
        This step saves:
        - preprocessing_steps metadata as JSON
        - raw, epochs, ica objects as pickle files
        - Additional metadata (subject, task, session, acquisition, events, event_id)
        ...
        step_config : dict
            Configuration for this step (currently unused)
            
        Returns
        -------
        data : dict
            Updated data dictionary with 'intermediate_results_path' key
        """
        # Create intermediate results directory
        deriv_root = self.bids_root / "derivatives" / "nice_preprocessing" / "intermediate"
        
        # Build unique identifier for this recording
        subject = data['subject']
        task = data['task']
        session = data.get('session', None)
        acquisition = data.get('acquisition', None)
        ...
        dir_name_parts = [f"sub-{subject}"]
        if session:
            dir_name_parts.append(f"ses-{session}")
        dir_name_parts.append(f"task-{task}")
        if acquisition:
            dir_name_parts.append(f"acq-{acquisition}")
        dir_name = "_".join(dir_name_parts)
        
        results_dir = deriv_root / dir_name
        results_dir.mkdir(parents=True, exist_ok=True)
        
        logger.info(f"Saving intermediate results to: {results_dir}")
        
        # Save preprocessing steps metadata as JSON
        metadata = {
            'subject': subject,
            'task': task,
            'session': session,
        ...
        if 'events' in data and data['events'] is not None:
            metadata['events'] = data['events'].tolist()
            metadata['events_sfreq'] = data.get('events_sfreq', None)
        if 'event_id' in data:
        ...
        if 'raw' in data and data['raw'] is not None:
            raw_file = results_dir / 'raw.pkl'
            with open(raw_file, 'wb') as f:
                pickle.dump(data['raw'], f)
            saved_objects.append('raw')
            logger.info(f"  Saved raw to: {raw_file}")
        
        if 'epochs' in data and data['epochs'] is not None:
            epochs_file = results_dir / 'epochs.pkl'
            with open(epochs_file, 'wb') as f:
                pickle.dump(data['epochs'], f)
            saved_objects.append('epochs')
            logger.info(f"  Saved epochs to: {epochs_file}")
        
        if 'ica' in data and data['ica'] is not None:
            ica_file = results_dir / 'ica.pkl'
            with open(ica_file, 'wb') as f:
                pickle.dump(data['ica'], f)
            saved_objects.append('ica')
            logger.info(f"  Saved ICA to: {ica_file}")
        
        # Store path in data
        data['intermediate_results_path'] = str(results_dir)
        
        # Add to preprocessing steps
        data['preprocessing_steps'].append({
            'step': 'save_intermediate_results',
            'path': str(results_dir),
            'saved_objects': saved_objects,
        })
        
        logger.info(f"Successfully saved intermediate results with objects: {saved_objects}")
        return data
    ...
    def _process_single_recording(self, bids_path: list[BIDSPath], progress: Progress = None, task_id: int = None) -> Dict[str, Any]:
        ...
        for key in ['raw_file', 'epochs_file', 'intermediate_results_path', 'n_epochs', 'preprocessing_steps']:
            if key in data:
