#!/usr/bin/env python3
"""
Command-Line Interface for EEG Preprocessing Pipeline (Preprocessing Only).

This module provides a command-line interface for running only the preprocessing
steps of the pipeline on BIDS-formatted datasets. Report generation is handled
separately by the report_cli.py module.

Main Functions
--------------
preprocessing_main() : Entry point for the preprocessing CLI
...
------------------
Basic usage:
    python src/preprocessing_cli.py --bids-root /path/to/bids --config config.yaml

With subject and task filtering:
    python src/preprocessing_cli.py --bids-root /path/to/bids --subjects 01 02 --tasks rest
...
def _parse_args():
    parser = argparse.ArgumentParser(description='Run EEG preprocessing pipeline (preprocessing only).')
...
def preprocessing_main():
    ...
    logger.info("Starting EEG preprocessing pipeline (preprocessing only)")
    ...
    pipeline = PreprocessingPipeline(bids_root=args.bids_root, output_root=args.output_root, config=config)
    ...
    for subject, subject_results in results.items():
        ...
        for result in subject_results:
            if 'error' in result:
                ...
                intermediate_path = result.get('intermediate_results_path', 'N/A')
                logger.info(f"  - Task: {task}, Session: {session}, Epochs: {n_epochs}")
                logger.info(f"    Intermediate results: {intermediate_path}")
    
    # Also write results to JSON file for easy processing
    output_json = Path(args.bids_root) / "derivatives" / "nice_preprocessing" / "preprocessing_results.json"
